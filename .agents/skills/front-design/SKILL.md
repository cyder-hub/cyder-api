...
### 4.6 弹窗与抽屉 (Dialog & Drawer/Sheet)

后台界面需高度重视上下文保持（Context-preserving），不要让复杂子实体的操作轻易跳转新页面或堆叠在无尽的超长页面中。这能有效解决“弹窗不足”导致的交互割裂感。

- **Dialog (居中对话框)**:
  - 适用：简单的危险确认（删除）、极少字段的快速输入（如选择框）、信息提示。
  - 尺寸：简单 `max-w-lg`，复杂上限 `max-w-3xl`。
- **Drawer / Sheet (侧边/底部抽屉)**:
  - 适用：子实体的新建与编辑（如在 Provider 中独立编辑 Model）、包含说明的复杂表单、向导流程。
  - 桌面端：优先通过 `vaul-vue` 的 `Drawer` 组件配合 `direction="right"` 实现从右侧划出的面板（即 Sheet），以便在不离开父级列表/页面的前提下完成深层操作。
  - 移动端：保持 `direction="bottom"` 作为底部抽屉，避免屏幕局促。
- 无论 Dialog 还是 Drawer，在移动端必须考虑：
  - 宽度占满或留出安全边距
  - 内容区必须可独立滚动，不穿透到底层
  - 底部操作按钮必须固定悬浮，稳定可见
...
- 对于包含多维度配置的页面（如 `ProviderEditPage`），应避免单一的垂直长滚动。
- 如果区块超过 3 个或高度超过 2 屏，应优先引入 `Tabs` 组件将内容分类（例如分为：基本信息 / 包含模型 / 凭证关联 / 高级设置）。
- 对于极其复杂的关联聚合页，也可以考虑“左侧次级导航 + 右侧滚动区”的经典控制台模式。
