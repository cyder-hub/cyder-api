...
export type RecordDiagnosticsTranslator = (
  key: string,
  params?: Record<string, unknown>,
) => string;

export interface UseRecordDiagnosticsOptions {
  t: RecordDiagnosticsTranslator;
  api: {
...
export function useRecordDiagnostics(options: UseRecordDiagnosticsOptions) {
  const api = options.api;
  const retentionPreview = ref<RecordDiagnosticsRetentionResponse | null>(null);
  const retentionLoading = ref(false);
  const retentionError = ref<string | null>(null);
  const storageInventory = ref<RecordDiagnosticsStorageInventoryResponse | null>(
    null,
  );
  const storageInventoryLoading = ref(false);
  const storageInventoryError = ref<string | null>(null);

  const previewRetention = async (payload: RecordDiagnosticsRetentionParams) => {
    retentionLoading.value = true;
    retentionError.value = null;
    try {
      retentionPreview.value = await api.previewRecordRetention(payload);
      return retentionPreview.value;
    } catch (err) {
      retentionError.value = normalizeError(
        err,
        options.t("recordPage.diagnostics.retentionFailed"),
      ).message;
      throw err;
    } finally {
  ...
  const executeRetention = async (payload: RecordDiagnosticsRetentionParams) => {
    retentionLoading.value = true;
    retentionError.value = null;
    try {
      retentionPreview.value = await api.executeRecordRetention(payload);
      return retentionPreview.value;
    } catch (err) {
      retentionError.value = normalizeError(
        err,
  ...
  const previewStorageInventory = async (
    payload: RecordDiagnosticsStorageInventoryParams,
  ) => {
    storageInventoryLoading.value = true;
    storageInventoryError.value = null;
    try {
      storageInventory.value = await api.previewRecordStorageInventory(payload);
      return storageInventory.value;
    } catch (err) {
      storageInventoryError.value = normalizeError(
        err,
        options.t("recordPage.diagnostics.storageInventoryFailed"),
      ).message;
      throw err;
    } finally {
      storageInventoryLoading.value = false;
    }
  };
