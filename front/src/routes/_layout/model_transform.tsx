...
// --- Component ---
export const Route = createFileRoute('/_layout/model_transform')({
    component: function ModelAliasPage() {
        const [transforms, { refetch: refetchTransforms }] = createResource(fetchModelAliassAPI);
        ...
        return (
            <div class="p-4">
                ...
                <Show when={transforms.error}>
                    <div class="text-center py-4 text-red-500">{t('modelAliasPage.errorPrefix')} {(transforms.error as Error)?.message}</div>
                </Show>
                                ...
                                <For each={transforms()}>{(transform) =>
                                    <TableRow>
                                        ...
                                        <TableCell class="space-x-2">
                                            <Button type="text" onClick={() => handleOpenEditModal(transform.id)} variant="primary" size="sm">{t('edit')}</Button>
                                            <Button type="text" onClick={() => handleDeleteTransform(transform.id, transform.alias_name)} variant="destructive" size="sm">{t('delete')}</Button>
                                        </TableCell>
                                    </TableRow>
                                }</For>
                            </TableBody>
