...
DROP TYPE IF EXISTS request_patch_operation_enum;
DROP TYPE IF EXISTS request_patch_placement_enum;
...
CREATE TABLE IF NOT EXISTS custom_field_definition (
    ...
    field_placement field_placement_enum NOT NULL,
    ...
    CONSTRAINT chk_cfd_value_type_coherence CHECK (
        (field_type = 'STRING'::field_type_enum AND string_value IS NOT NULL AND integer_value IS NULL AND number_value IS NULL AND boolean_value IS NULL)
            OR
        (field_type = 'INTEGER'::field_type_enum AND string_value IS NULL AND integer_value IS NOT NULL AND number_value IS NULL AND boolean_value IS NULL)
            OR
        (field_type = 'NUMBER'::field_type_enum AND string_value IS NULL AND integer_value IS NULL AND number_value IS NOT NULL AND boolean_value IS NULL)
            OR
        (field_type = 'BOOLEAN'::field_type_enum AND string_value IS NULL AND integer_value IS NULL AND number_value IS NULL AND boolean_value IS NOT NULL)
            OR
        (field_type = 'JSON_STRING'::field_type_enum AND string_value IS NOT NULL AND integer_value IS NULL AND number_value IS NULL AND boolean_value IS NULL)
            OR
        (field_type = 'UNSET'::field_type_enum AND string_value IS NULL AND integer_value IS NULL AND number_value IS NULL AND boolean_value IS NULL)
    ),
