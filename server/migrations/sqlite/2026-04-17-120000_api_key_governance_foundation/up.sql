...
SELECT
    ...
    sak.api_key,
    NULL,
    substr(sak.api_key, 1, 12),
    CASE
        WHEN LENGTH(sak.api_key) <= 4 THEN sak.api_key
        ELSE substr(sak.api_key, -4)
    END,
    sak.name,
    sak.description,
    COALESCE(acp.default_action, 'ALLOW'),
    sak.is_enabled,
    NULL,
    NULL,
    NULL,
...
INSERT OR IGNORE INTO api_key_acl_rule (
    id,
    api_key_id,
    effect,
    scope,
...
SELECT
    -ROW_NUMBER() OVER (ORDER BY sak.id, acr.priority, acr.id),
    sak.id,
    acr.rule_type,
    acr.scope,
    acr.provider_id,
