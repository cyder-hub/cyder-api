...
CREATE TABLE provider_without_reasoning_profile (
...
INSERT INTO provider_without_reasoning_profile (
    id,
    provider_key,
    name,
    endpoint,
...
DROP TABLE provider;
ALTER TABLE provider_without_reasoning_profile RENAME TO provider;

CREATE UNIQUE INDEX IF NOT EXISTS idx_provider_key_unique_when_active
    ON provider (provider_key)
    WHERE deleted_at IS NULL AND is_enabled = true;

CREATE TABLE model_without_reasoning_profile (
    id BIGINT PRIMARY KEY NOT NULL,
...
INSERT INTO model_without_reasoning_profile (
    id,
    provider_id,
    cost_catalog_id,
    model_name,
...
DROP TABLE model;
ALTER TABLE model_without_reasoning_profile RENAME TO model;
