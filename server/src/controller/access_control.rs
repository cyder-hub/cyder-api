use crate::database::{
    ...
    access_control::{
        AccessControlPolicy, // Renamed
        ApiAccessControlPolicy,
        ApiCreateAccessControlPolicyPayload,
        ApiUpdateAccessControlPolicyPayload,
    },
};
...
use std::sync::Arc; // Added Arc for AppState // Added AppState
...
async fn list() -> Result<(StatusCode, HttpResult<Vec<ApiAccessControlPolicy>>), BaseError> {
    // Renamed
...
async fn get_one(Path(id): Path<i64>) -> Result<HttpResult<ApiAccessControlPolicy>, BaseError> {
    // Renamed
    // AccessControlPolicy::get_by_id returns ApiAccessControlPolicy
    match AccessControlPolicy::get_by_id(id) {
        // Renamed
...
async fn update_one(
    ...
    if let Err(e) = app_state.invalidate_access_control_policy(id).await {
        ...
        warn!(
            "Failed to invalidate AccessControlPolicy id {} from cache: {:?}",
            id, e
        );
    }
...
pub fn create_access_control_policy_router() -> StateRouter {
    // Renamed function
