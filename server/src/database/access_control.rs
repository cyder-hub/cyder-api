...
pub struct ApiAccessControlRule {
    // Renamed from ApiLimitStrategyItem
...
pub struct ApiCreateAccessControlPolicyPayload {
    // Renamed
    pub name: String,
    pub description: Option<String>,
...
pub struct ApiUpdateAccessControlPolicyPayload {
    // Renamed
    pub name: Option<String>,
    pub description: Option<Option<String>>,
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ApiAccessControlPolicy {
    // Renamed
    pub id: i64,
    pub name: String,
    pub description: Option<String>,
...
impl AccessControlPolicy {
    ...
    fn insert_rules_batch(
        // Renamed from insert_items_batch
            ...
            diesel::insert_into(access_control_rule::table) // Updated table
                ...
                .map_err(|e| {
                    BaseError::DatabaseFatal(Some(format!(
                        "Failed to insert access control rules: {}",
                        e
                    )))
                })?;
            Ok(())
    ...
    fn delete_rules_by_policy_id(
        // Renamed from delete_items_by_strategy_id
        conn: &mut DbConnection,
    ...
    pub fn create(
        payload: ApiCreateAccessControlPolicyPayload,
    ) -> DbResult<ApiAccessControlPolicy> {
        // Renamed
        let conn = &mut get_connection()?;
        let now = Utc::now().timestamp_millis();
        let policy_id = ID_GENERATOR.generate_id(); // Renamed

        let new_policy_db = DbNewAccessControlPolicy {
            // Renamed
            id: policy_id,
            name: payload.name,
    ...
    pub fn update(
        ...
        let update_policy_data = DbUpdateAccessControlPolicy {
            // Renamed
            name: payload.name,
            description: payload.description,
            default_action: payload.default_action,
    ...
    pub fn delete(policy_id: i64) -> DbResult<usize> {
        // Renamed
        let conn = &mut get_connection()?;
        let now = Utc::now().timestamp_millis();

        // Soft delete the policy
        let num_deleted_policy = db_execute!(conn, {
            // Renamed
            diesel::update(access_control_policy::table.find(policy_id)) // Updated table
                .set((
        ...
        if num_deleted_policy > 0 {
            ...
            db_execute!(conn, {
                diesel::update(
                    access_control_rule::table
                        .filter(access_control_rule::dsl::policy_id.eq(policy_id)),
                )
                .set((
                ...
                .map_err(|e| {
                    BaseError::DatabaseFatal(Some(format!(
                        "Failed to soft-delete rules for policy {}: {}",
                        policy_id, e
                    )))
                })
            })?;
    ...
    pub fn get_by_id(policy_id: i64) -> DbResult<ApiAccessControlPolicy> {
        // Renamed
        let conn = &mut get_connection()?;
        let policy: AccessControlPolicy = db_execute!(conn, {
            // Renamed
            access_control_policy::table // Updated table
                ...
                .map_err(|e| match e {
                    diesel::result::Error::NotFound => {
                        BaseError::NotFound(Some(format!("Policy {} not found", policy_id)))
                    }
        ...
        let all_rules: Vec<AccessControlRule> = db_execute!(conn, {
            // Renamed
            access_control_rule::table // Updated table
    ...
    pub fn list_all() -> DbResult<Vec<ApiAccessControlPolicy>> {
        // Renamed
        let conn = &mut get_connection()?;
        let policies: Vec<AccessControlPolicy> = db_execute!(conn, {
            // Renamed
            access_control_policy::table // Updated table
                .filter(access_control_policy::dsl::deleted_at.is_null()) // Updated field
        ...
        for policy in policies {
            ...
            result.push(ApiAccessControlPolicy {
                // Renamed
                id: policy.id,
