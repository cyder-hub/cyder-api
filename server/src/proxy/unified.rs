...
/// Unified proxy handler for OpenAI, Anthropic, Responses, and Ollama API types.
///
/// This function encapsulates the common proxy flow shared across all non-Gemini handlers:
/// authenticate → parse body → resolve model → transform → access control → prepare → log → proxy.
///
/// The `api_type` parameter determines the authentication method and request/response format.
