    ...
    T: Serialize + DeserializeOwned + Send + Sync + Clone + 'static,
{
    ...
    async fn mset(
        ...
        for (key, value) in entries {
            self.data
                .insert(key.to_string(), (value.clone(), expiration));
            self.metrics.record_set();
        }
...
mod tests {
    ...
    async fn test_mget_mset() {
        ...
        let entries = vec![
            (
                "key1",
                Arc::new(CacheEntry::Positive(Arc::new("value1".to_string()))),
            ),
            (
                "key2",
                Arc::new(CacheEntry::Positive(Arc::new("value2".to_string()))),
            ),
