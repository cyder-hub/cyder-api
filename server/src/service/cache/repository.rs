    ...
    B: CacheBackend<T>,
{
    ...
    pub async fn get_entry(
        &self,
        cache_key: &str,
    ) -> Result<Option<Arc<CacheEntry<T>>>, CacheError> {
        self.backend
            .get(cache_key)
            .await
    ...
    pub async fn set_positive(&self, cache_key: &str, value: &T) -> Result<(), CacheError> {
        ...
        self.backend
            .set(cache_key, entry, self.default_ttl)
            .await
            .map_err(|e| CacheError::BackendError(e.to_string()))
    }
    ...
    pub async fn mset_positive(&self, entries: &[(&str, &T)]) -> Result<(), CacheError> {
        let arc_entries: Vec<(&str, Arc<CacheEntry<T>>)> = entries
            .iter()
            .map(|(key, value)| {
                let entry = Arc::new(CacheEntry::Positive(Arc::new((*value).clone())));
                (*key, entry)
        ...
        self.backend
            .mset(&arc_entries, self.default_ttl)
            .await
            .map_err(|e| CacheError::BackendError(e.to_string()))
    }
}
...
mod tests {
    ...
    async fn test_get_set_negative() {
        ...
        repo.set_negative("item:id:2", Duration::from_secs(60))
            .await
            .unwrap();
    ...
    async fn test_multiple_dimensions() {
        ...
        repo.set_positive("item:id:1", &item).await.unwrap();
        repo.set_positive("item:name:test_item", &item)
            .await
            .unwrap();
    ...
    async fn test_mget_mset() {
        ...
        let entries = vec![("item:id:1", &items[0]), ("item:id:2", &items[1])];
        repo.mset_positive(&entries).await.unwrap();
        repo.set_negative("item:id:3", Duration::from_secs(60))
            .await
            .unwrap();
